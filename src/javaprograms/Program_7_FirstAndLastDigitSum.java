...
public class Program_7_FirstAndLastDigitSum {
    public static int sumFirstAndLastDigit(int number) {
        ...
        int r, reverse = 0, firstd =0, lastd =0, sum =0;
        // lastd = number % 10;
        while (number > 0) {
            lastd = number % 10;     // logic to find last digit of the number.
            reverse = reverse * 10 + lastd;   // now reverse the number, mulitiply by 10 and then add
                                               //     last digit we obtained above.
            r = reverse / 10;     // now devided by 10 to remove the last digit to balance the logic
            firstd = r % 10;
        }
           // firstd = r % 10;    // now modulus the variable r by 10 (% 10) so we can get first digit.

            sum = firstd + lastd;   // now sum first digit and last digit
           return sum;              // returm sum.
